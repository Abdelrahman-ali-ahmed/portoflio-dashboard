...
const SideBar = () => {
  const { open, setOpen, login, Menus, location } = useSideBar();
  ...
  return (
    <div
      className={`${
        open ? "w-72" : "w-20"
      } bg-dark-purple dark:bg-gray-800 h-screen fixed top-0 left-0 p-5 pt-8 z-50 duration-300`}
    >
      <img
        ...
        {Menus.map((Menu, index) => (
          <li
            ...
            className={`flex rounded-md p-2 cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-700 
              hover:text-blue-500 text-gray-300 dark:text-gray-200 text-sm items-center gap-x-4 ${location.pathname==`/${Menu.title.toLocaleLowerCase()}`?"text-blue-500 dark:text-blue-400 bg-gray-100 dark:bg-gray-700":""}`}
